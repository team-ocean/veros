...
def biogeochemistry(vs):
    ...
    #phyto_integrated = np.zeros_like(vs.phytoplankton[:, :, 0, vs.tau])
    ...
    for flux in surfaces_fluxes:
        for key, value in flux.items():
            ...
            #vs.npzd_tracers[key][:, :, -1, vs.tau] += value * vs.dt_mom / vs.dzt[-1]
    ...
    for k in reversed(range(vs.nz)):
        ...
        #tracers = {name: value[:, :, k, vs.tau] for name, value in vs.npzd_tracers.items()}
        ...
        for _ in range(nbio):
            ...
            for update in npzd_updates:
                for key, value in update.items():
                    ...
                    # vs.npzd_tracers[key][:, :, k, vs.tau] = vs.npzd_tracers[key][:, :, k, vs.tau] + value * vs.dt_bio  # NOTE far too many multiplications with dt
            ...
            for tracer in vs.sinking_speeds:
                ...
                # vs.npzd_tracers[tracer][:, :, k, vs.taup1] += impo[tracer] - tmp_expo[tracer]
            ...
            #    tracers[name] = vs.npzd_tracers[name][:, :, k, vs.tau]
        ...
        if vs.enable_carbon:  # TODO remove ifs in process code
            ...
            # vs.alkalinity[:, :, k, vs.tau] = - temporary_tracers["DIC"][:, :, k] / vs.redfield_ratio_CN# * 1e-3

            if not vs.enable_calcifiers:
                # vs.alkalinity[:, :, k] -= 2 * dprca
                #vs.alkalinity[:, :, k, vs.tau] -= 2 * dprca
        ...
        #     vs.npzd_tracers[name][:, :, k, vs.tau] = value
    ...
    if vs.enable_carbon:
        ...
        # temporary_tracers["DIC"][:, :, 1:] += vs.prca[:, :, np.newaxis] * vs.rcak[:, :, 1:]
        # temporary_tracers["DIC"][:, :, 0] += vs.prca * vs.rcab[:, :, 0]

        # vs.alkalinity[:, :, 1:, vs.tau] += 2 * vs.prca[:, :, np.newaxis] * vs.rcak[:, :, 1:]
        # vs.alkalinity[:, :, 0, vs.tau] += 2 * vs.prca * vs.rcab[:, :, 0]
        # temporary_tracers["alkalinity"][:, :, 1:] += 2 * vs.prca[:, :, np.newaxis] * vs.rcak[:, :, 1:]
        # temporary_tracers["alkalinity"][:, :, 0] += 2 * vs.prca * vs.rcab[:, :, 0]
...
def setupNPZD(vs):
    ...
    if vs.enable_carbon:
      ...
      # dcaco3 = 650000.0  ! remineralisation depth of calcite [cm]
        dcaco3 = 650000.0 / 100  # remineralization depth of calcite [m]
...
def npzd(vs):
    ...
    for tracer, tracer_data in vs.npzd_tracers.items():
        ...
        #     cyclic.setcyclic_x(tracer_data[:, :, :, vs.taup1])

        # TODO: calc_eq_of_state?

        # TODO: Surface density flux?
