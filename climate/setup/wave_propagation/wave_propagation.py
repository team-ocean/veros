...
class WavePropagation(PyOM):
    ...
    def set_diagnostics(self):
                self.enable_diag_ts_monitor = True
                self.ts_monint = 86400.0
                self.enable_diag_snapshots = True

                self.enable_diag_overturning = True
                self.overint = 365 * 86400
                self.overfreq = 365 * 86400 / 24.
                self.enable_diag_energy = True
                self.energint = 365 * 86400
                self.energfreq = 365 * 86400 / 24.
                self.enable_diag_averages = True
                self.aveint = 365 * 86400
                self.avefreq = 365 * 86400 / 24.
        m = self.main_module
        idm = self.idemix_module
        tkm = self.tke_module
        ekm = self.eke_module
