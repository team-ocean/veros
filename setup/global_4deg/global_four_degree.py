...
class GlobalFourDegree(VerosLegacy):
    ...
    def set_initial_conditions(self):
        ...
        m.temp[2:-2, 2:-2, :, :2] = temp_data[:, :, :, np.newaxis] * \
        ...
        fxa = np.sum(self.qnet[2:-2, 2:-2, :] * m.area_t[2:-2, 2:-2,
                                                         np.newaxis]) / 12 / np.sum(m.area_t[2:-2, 2:-2])
    ...
    def _get_periodic_interval(self, currentTime, cycleLength, recSpacing, nbRec):
        ...
        locTime = currentTime - recSpacing * 0.5 + \
            cycleLength * (2 - round(currentTime / cycleLength))
    ...
    def set_forcing(self):
        ...
        (n1, f1), (n2, f2) = self._get_periodic_interval(
            (m.itt - 1) * m.dt_tracer, 365 * 86400.0, 365 * 86400. / 12., 12)
        ...
        m.forc_temp_surface[:] = (
            qnet + qnec * (sst - m.temp[:, :, -1, self.get_tau()])) * m.maskT[:, :, -1] / cp_0 / m.rho_0
        ...
        mask = (m.temp[:, :, -1, self.get_tau()] * m.maskT[:, :, -1]
                <= -1.8) & (m.forc_temp_surface <= 0.0)
        m.forc_temp_surface[mask] = 0.0
        m.forc_salt_surface[mask] = 0.0

        if m.enable_tempsalt_sources:
            m.temp_source[:] = m.maskT * self.rest_tscl * \
                (f1 * self.t_star[:, :, :, n1] + f2 *
                 self.t_star[:, :, :, n2] - m.temp[:, :, :, self.get_tau()])
            m.salt_source[:] = m.maskT * self.rest_tscl * \
                (f1 * self.s_star[:, :, :, n1] + f2 *
                 self.s_star[:, :, :, n2] - m.salt[:, :, :, self.get_tau()])

    @veros_method
    def set_diagnostics(self):
        self.diagnostics["averages"].output_variables = [
            "temp", "salt", "u", "v", "w", "surface_taux", "surface_tauy", "psi"]
