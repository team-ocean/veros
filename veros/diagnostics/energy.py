...
ENERGY_VARIABLES = dict(
    ...
    # mean energy content
    k_m=Variable('Mean kinetic energy', None, 'J', 'Mean kinetic energy',
                 write_to_restart=True),
    Hd_m=Variable('Mean dynamic enthalpy', None, 'J', 'Mean dynamic enthalpy',
                  write_to_restart=True),
    eke_m=Variable('Meso-scale eddy energy', None, 'J', 'Meso-scale eddy energy',
                   write_to_restart=True),
    iw_m=Variable('Internal wave energy', None, 'J', 'Internal wave energy',
                  write_to_restart=True),
    tke_m=Variable('Turbulent kinetic energy', None, 'J', 'Turbulent kinetic energy',
                   write_to_restart=True),

    # energy changes
    dE_tot_m=Variable('Change of total energy', None, 'W', 'Change of total energy',
                      write_to_restart=True),
    dk_m=Variable('Change of KE', None, 'W', 'Change of kinetic energy',
                  write_to_restart=True),
    dHd_m=Variable('Change of Hd', None, 'W', 'Change of dynamic enthalpy',
                   write_to_restart=True),
    deke_m=Variable('Change of EKE', None, 'W', 'Change of meso-scale eddy energy',
                    write_to_restart=True),
    diw_m=Variable('Change of E_iw', None, 'W', 'Change of internal wave energy',
                   write_to_restart=True),
    dtke_m=Variable('Change of TKE', None, 'W', 'Change of tubulent kinetic energy',
                    write_to_restart=True),

    # dissipation
    ke_diss_m=Variable('Dissipation of KE', None, 'W', 'Dissipation of kinetic energy',
                       write_to_restart=True),
    Hd_diss_m=Variable('Dissipation of Hd', None, 'W', 'Dissipation of dynamic enthalpy',
                       write_to_restart=True),
    eke_diss_m=Variable('Dissipation of EKE', None, 'W', 'Dissipation of meso-scale eddy energy',
                        write_to_restart=True),
    iw_diss_m=Variable('Dissipation of E_iw', None, 'W', 'Dissipation of internal wave energy',
                       write_to_restart=True),
    tke_diss_m=Variable('Dissipation of TKE', None, 'W', 'Dissipation of turbulent kinetic energy',
                        write_to_restart=True),
    adv_diss_m=Variable('Dissipation by advection', None, 'W', 'Dissipation by advection',
                        write_to_restart=True),

    # external forcing
    wind_m=Variable('Wind work', None, 'W', 'Wind work',
                    write_to_restart=True),
    dHd_sources_m=Variable('Hd production by ext. sources', None, 'W',
                           'Dynamic enthalpy production through external sources',
                           write_to_restart=True),
    iw_forc_m=Variable('External forcing of E_iw', None, 'W',
                       'External forcing of internal wave energy',
                       write_to_restart=True),
    tke_forc_m=Variable('External forcing of TKE', None, 'W',
                        'External forcing of turbulent kinetic energy',
                        write_to_restart=True),

    # exchange
    ke_hd_m=Variable('Exchange KE -> Hd', None, 'W',
                     'Exchange between kinetic energy and dynamic enthalpy',
                     write_to_restart=True),
    ke_tke_m=Variable('Exchange KE -> TKE by vert. friction', None, 'W',
                      'Exchange between kinetic energy and turbulent kinetic energy by vertical friction',
                      write_to_restart=True),
    ke_iw_m=Variable('Exchange KE -> IW by bottom friction', None, 'W',
                     'Exchange between kinetic energy and internal wave energy by bottom friction',
                     write_to_restart=True),
    tke_hd_m=Variable('Exchange TKE -> Hd by vertical mixing', None, 'W',
                      'Exchange between turbulent kinetic energy and dynamic enthalpy by vertical mixing',
                      write_to_restart=True),
    ke_eke_m=Variable('Exchange KE -> EKE by lateral friction', None, 'W',
                      'Exchange between kinetic energy and eddy kinetic energy by lateral friction',
                      write_to_restart=True),
    hd_eke_m=Variable('Exchange Hd -> EKE by GM and lateral mixing', None, 'W',
                      'Exchange between dynamic enthalpy and eddy kinetic energy by GM and lateral mixing',
                      write_to_restart=True),
    eke_tke_m=Variable('Exchange EKE -> TKE', None, 'W',
                       'Exchange between eddy and turbulent kinetic energy',
                       write_to_restart=True),
    eke_iw_m=Variable('Exchange EKE -> IW', None, 'W',
                      'Exchange between eddy kinetic energy and internal wave energy',
                      write_to_restart=True),

    # cabbeling
    cabb_m=Variable('Cabbeling by vertical mixing', None, 'W',
                    'Cabbeling by vertical mixing',
                    write_to_restart=True),
    cabb_iso_m=Variable('Cabbeling by isopycnal mixing', None, 'W',
