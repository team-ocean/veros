...
def momentum(veros):
    ...
    veros.du_cor[2:-2, 2:-2] = veros.maskU[2:-2, 2:-2] \
            ...
            * veros.dxt[2:-2,np.newaxis,np.newaxis] / veros.dxu[2:-2,np.newaxis,np.newaxis] \
            + veros.coriolis_t[3:-1,2:-2,np.newaxis] * (veros.v[3:-1,2:-2,:,veros.tau] + veros.v[3:-1,1:-3,:,veros.tau]) \
    ...
    veros.dv_cor[2:-2, 2:-2] = -veros.maskV[2:-2, 2:-2]\
            ...
            * veros.dyt[np.newaxis,2:-2,np.newaxis] * veros.cost[np.newaxis,2:-2,np.newaxis] \
            ...
            + veros.coriolis_t[2:-2,3:-1,np.newaxis] * (veros.u[1:-3, 3:-1,:,veros.tau] + veros.u[2:-2,3:-1,:,veros.tau]) \
    ...
    if veros.coord_degree:
        ...
        veros.dv_cor[2:-2, 2:-2] += -veros.maskV[2:-2, 2:-2] * 0.125 \
                ...
                * veros.dyt[np.newaxis,2:-2,np.newaxis] * veros.cost[np.newaxis,2:-2,np.newaxis] \
                / (veros.dyu[np.newaxis,2:-2,np.newaxis] * veros.cosu[np.newaxis,2:-2,np.newaxis]) \
                + veros.tantr[np.newaxis,3:-1,np.newaxis] * (veros.u[2:-2,3:-1,:,veros.tau] + veros.u[1:-3,3:-1,:,veros.tau])**2 \
                * veros.dyt[np.newaxis,3:-1,np.newaxis] * veros.cost[np.newaxis,3:-1,np.newaxis] \
                / (veros.dyu[np.newaxis,2:-2,np.newaxis] * veros.cosu[np.newaxis,2:-2,np.newaxis]))
    ...
    if not veros.enable_hydrostatic:
        ...
        #        veros.dw[i,j,:,veros.tau] = veros.dw_cor[i,j,:]

    """
    wind stress forcing
    """
    veros.du[2:-2,2:-2,-1,veros.tau] += veros.maskU[2:-2,2:-2,-1] * veros.surface_taux[2:-2,2:-2] / veros.dzt[-1]
    veros.dv[2:-2,2:-2,-1,veros.tau] += veros.maskV[2:-2,2:-2,-1] * veros.surface_tauy[2:-2,2:-2] / veros.dzt[-1]
...
def vertical_velocity(veros):
    ...
    fxa[:,:,0] = -veros.maskW[1:,1:,0] * veros.dzt[0] * \
          ((veros.u[1:,1:,0,veros.taup1]-veros.u[:-1,1:,0,veros.taup1])/(veros.cost[np.newaxis,1:]*veros.dxt[1:,np.newaxis]) \
          + (veros.cosu[np.newaxis,1:] * veros.v[1:,1:,0,veros.taup1] - veros.cosu[np.newaxis,:-1] * veros.v[1:,:-1,0,veros.taup1]) / (veros.cost[np.newaxis, 1:] * veros.dyt[np.newaxis, 1:]))
...
def momentum_advection(veros):
    ...
    utr = veros.u[...,veros.tau] * veros.maskU * veros.dyt[np.newaxis, :, np.newaxis] * veros.dzt[np.newaxis, np.newaxis, :]
    vtr = veros.dzt[np.newaxis, np.newaxis, :] * veros.cosu[np.newaxis, :, np.newaxis] * veros.dxt[:, np.newaxis, np.newaxis] * veros.v[...,veros.tau] * veros.maskV
    wtr = veros.w[...,veros.tau] * veros.maskW * veros.area_t[:,:,np.newaxis]

    """
    for zonal momentum
    """
    veros.flux_east[1:-2, 2:-2] = 0.25*(veros.u[1:-2,2:-2,:,veros.tau]+veros.u[2:-1,2:-2,:,veros.tau])*(utr[2:-1,2:-2]+utr[1:-2,2:-2])
    veros.flux_north[2:-2,1:-2] = 0.25*(veros.u[2:-2,1:-2,:,veros.tau]+veros.u[2:-2,2:-1,:,veros.tau])*(vtr[3:-1,1:-2]+vtr[2:-2,1:-2])
    veros.flux_top[2:-2,2:-2,:-1] = 0.25*(veros.u[2:-2,2:-2,1:,veros.tau]+veros.u[2:-2,2:-2,:-1,veros.tau])*(wtr[2:-2,2:-2,:-1]+wtr[3:-1,2:-2,:-1])
    veros.flux_top[:,:,-1] = 0.0
    veros.du_adv[2:-2,2:-2] = -veros.maskU[2:-2,2:-2] * (veros.flux_east[2:-2,2:-2] - veros.flux_east[1:-3,2:-2] \
            + veros.flux_north[2:-2,2:-2]-veros.flux_north[2:-2,1:-3])/(veros.dzt[np.newaxis,np.newaxis,:]*veros.area_u[2:-2,2:-2,np.newaxis])
    ...
    """
    veros.flux_east[1:-2, 2:-2] = 0.25 *(veros.v[1:-2,2:-2,:,veros.tau]+veros.v[2:-1,2:-2,:,veros.tau])*(utr[1:-2,3:-1]+utr[1:-2,2:-2])
    veros.flux_north[2:-2, 1:-2] = 0.25 * (veros.v[2:-2,1:-2,:,veros.tau]+veros.v[2:-2,2:-1,:,veros.tau])*(vtr[2:-2,2:-1]+vtr[2:-2,1:-2])
    veros.flux_top[2:-2,2:-2,:-1] = 0.25*(veros.v[2:-2,2:-2,1:,veros.tau]+veros.v[2:-2,2:-2,:-1,veros.tau])*(wtr[2:-2,2:-2,:-1]+wtr[2:-2,3:-1,:-1])
    veros.flux_top[:,:,veros.nz-1] = 0.0
    veros.dv_adv[2:-2,2:-2] = -veros.maskV[2:-2,2:-2]*(veros.flux_east[2:-2,2:-2] - veros.flux_east[1:-3,2:-2] \
            + veros.flux_north[2:-2,2:-2] - veros.flux_north[2:-2,1:-3])/(veros.dzt*veros.area_v[2:-2,2:-2,np.newaxis])
    tmp = veros.dzt * veros.area_v[:,:,np.newaxis]
    veros.dv_adv[:,:,0] += -veros.maskV[:,:,0]*veros.flux_top[:,:,0]/tmp[:,:,0]
    veros.dv_adv[:,:,1:] += -veros.maskV[:,:,1:]*(veros.flux_top[:,:,1:]-veros.flux_top[:,:,:-1])/tmp[:,:,1:]
    ...
    if not veros.enable_hydrostatic:
        ...
        veros.dw_adv[:,:,1:] -= tmp[:,:,1:] * (veros.flux_top[:,:,1:] - veros.flux_top[:,:,:-1])
