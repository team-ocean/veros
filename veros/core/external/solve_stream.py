...
def solve_streamfunction(veros):
    ...
    tmp[:,:,:-1] += 0.5 * veros.rho[:,:,1:,veros.tau] * fxa * veros.dzw[:-1] * veros.maskT[:,:,:-1]
    veros.p_hydro[:,:,-2::-1] = veros.maskT[:,:,-2::-1] * (veros.p_hydro[:,:,-1,np.newaxis] + np.cumsum(tmp[:,:,-2::-1], axis=2))
    ...
    fpx = np.sum((veros.du[:,:,:,veros.tau] + veros.du_mix) * veros.maskU * veros.dzt, axis=(2,)) * veros.hur
    fpy = np.sum((veros.dv[:,:,:,veros.tau] + veros.dv_mix) * veros.maskV * veros.dzt, axis=(2,)) * veros.hvr
    ...
    veros.dpsi[:,:,veros.taup1] = 2 * veros.dpsi[:,:,veros.tau] - veros.dpsi[:,:,veros.taum1] # first guess, we need three time levels here
    ...
    if veros.nisle > 1:
        ...
        line_forc[1:] = utilities.line_integrals(veros, fpx[...,np.newaxis], fpy[...,np.newaxis], kind="same")[1:]
    ...
    veros.u[:,:,:,veros.taup1] += -fpx[:, :, np.newaxis] * veros.maskU * veros.hur[:, :, np.newaxis]
    veros.v[:,:,:,veros.taup1] += -fpy[:, :, np.newaxis] * veros.maskV * veros.hvr[:, :, np.newaxis]
