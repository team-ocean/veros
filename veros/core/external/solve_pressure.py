...
def solve_pressure(veros):
    ...
    for j in range(veros.js_pe, veros.je_pe + 1):  # j=veros.js_pe,veros.je_pe
        for i in range(veros.is_pe, veros.ie_pe):  # i=veros.is_pe,veros.ie_pe
            ...
            veros.dv[i, j, :, veros.tau] -= (veros.p_hydro[i, j + 1, :] -
                                             veros.p_hydro[i, j, :]) / veros.dyu[j] * veros.maskV[i, j, :]
        ...
        for j in range(veros.js_pe, veros.je_pe):  # j=veros.js_pe,veros.je_pe
            for i in range(veros.is_pe, veros.ie_pe):  # i=veros.is_pe,veros.ie_pe
                ...
                fpy[i, j] += veros.v[i, j, k, veros.taup1] * \
                    veros.maskV[i, j, k] * veros.dzt[k] / veros.dt_mom
    ...
    veros.psi[:, :, veros.taup1] = 2 * veros.psi[:, :, veros.tau] - \
        veros.psi[:, :, veros.taum1]  # first guess
    ...
    for j in range(veros.js_pe, veros.je_pe + 1):  # j=veros.js_pe,veros.je_pe
        for i in range(veros.is_pe, veros.ie_pe + 1):  # i=veros.is_pe,veros.ie_pe
            ...
            veros.v[i, j, :, veros.taup1] -= veros.dt_mom * \
                (veros.psi[i, j + 1, veros.taup1] - veros.psi[i, j, veros.taup1]) / \
...
def make_coeff_surf_press(veros):
    ...
    for j in range(veros.js_pe, veros.je_pe + 1):  # j=veros.js_pe,veros.je_pe
        for i in range(veros.is_pe, veros.ie_pe + 1):  # i=veros.is_pe,veros.ie_pe
            ...
            cf[i, j, 1, 1] -= mm * veros.hu[i - 1, j] / \
                veros.dxu[i - 1] / veros.dxt[i] / veros.cost[j]**2
            cf[i, j, 0, 1] += mm * veros.hu[i - 1, j] / \
                veros.dxu[i - 1] / veros.dxt[i] / veros.cost[j]**2

            mp = maskM[i, j] * maskM[i, j + 1]
            mm = maskM[i, j] * maskM[i, j - 1]
            cf[i, j, 1, 1] -= mp * veros.hv[i, j] / veros.dyu[j] / \
                veros.dyt[j] * veros.cosu[j] / veros.cost[j]
            cf[i, j, 1, 2] += mp * veros.hv[i, j] / veros.dyu[j] / \
                veros.dyt[j] * veros.cosu[j] / veros.cost[j]
            cf[i, j, 1, 1] -= mm * veros.hv[i, j - 1] / veros.dyu[j - 1] / \
                veros.dyt[j] * veros.cosu[j - 1] / veros.cost[j]
            cf[i, j, 1, 0] += mm * veros.hv[i, j - 1] / veros.dyu[j - 1] / \
                veros.dyt[j] * veros.cosu[j - 1] / veros.cost[j]
