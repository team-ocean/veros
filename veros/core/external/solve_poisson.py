...
def solve(veros, rhs, sol, boundary_val=None):
    ...
    """
    veros.flush()
    if not solve.veros or solve.veros != id(veros): # only initialize solver if parent object changes
        if has_pyamg:
...
def _assemble_poisson_matrix(veros):
    ...
    east_diag, west_diag, north_diag, south_diag = (np.zeros((veros.nx+4, veros.ny+4)) for _ in range(4))
    ...
    east_diag[2:-2, 2:-2] = veros.hvr[3:-1, 2:-2] / veros.dxu[2:-2, np.newaxis] / veros.dxt[3:-1, np.newaxis] / veros.cosu[np.newaxis, 2:-2]**2
    west_diag[2:-2, 2:-2] = veros.hvr[2:-2, 2:-2] / veros.dxu[2:-2, np.newaxis] / veros.dxt[2:-2, np.newaxis] / veros.cosu[np.newaxis, 2:-2]**2
    north_diag[2:-2, 2:-2] = veros.hur[2:-2, 3:-1] / veros.dyu[np.newaxis, 2:-2] / veros.dyt[np.newaxis, 3:-1] * veros.cost[np.newaxis, 3:-1] / veros.cosu[np.newaxis, 2:-2]
    south_diag[2:-2, 2:-2] = veros.hur[2:-2, 2:-2] / veros.dyu[np.newaxis, 2:-2] / veros.dyt[np.newaxis, 2:-2] * veros.cost[np.newaxis, 2:-2] / veros.cosu[np.newaxis, 2:-2]
