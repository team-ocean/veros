...
def line_integrals(veros,uloc,vloc,kind="same"):
    ...
    south = -vloc[2:-1,1:-2,:] * veros.dyu[np.newaxis, 1:-2, np.newaxis] \
                                + uloc[1:-2,2:-1,:] \
                                    * veros.dxu[1:-2, np.newaxis, np.newaxis] \
                                    * veros.cost[np.newaxis,2:-1,np.newaxis]
    east = np.sum(east[s1] * (veros.line_dir_east_mask[1:-2,1:-2] & veros.boundary_mask[1:-2,1:-2])[s2], axis=(0,1))
    west = np.sum(west[s1] * (veros.line_dir_west_mask[1:-2,1:-2] & veros.boundary_mask[1:-2,1:-2])[s2], axis=(0,1))
    north = np.sum(north[s1] * (veros.line_dir_north_mask[1:-2,1:-2] & veros.boundary_mask[1:-2,1:-2])[s2], axis=(0,1))
    south = np.sum(south[s1] * (veros.line_dir_south_mask[1:-2,1:-2] & veros.boundary_mask[1:-2,1:-2])[s2], axis=(0,1))
...
def apply_op(veros, cf, p1, res):
    ...
    res[2:veros.nx+2, 2:veros.ny+2] = np.sum(cf[2:veros.nx+2, 2:veros.ny+2] * P1, axis=(2,3))
