...
@veros_method
def isleperim(veros, kmt, boundary_map, iperm, jperm, iofs, nippts, imt, jmt, mnisle, maxipp, change_nisle=False, verbose=False):
    """
    ...
    if veros.enable_cyclic_x and verbose:
    ...
    """
    veros.flush() # sync before assigning values to numpy array boundary_map
    ...
    if verbose:
        ...
        print " number of land masses is ", veros.nisle
        print " number of island perimeter points is ",  nippts[veros.nisle] + iofs[veros.nisle]

@veros_method
def expand(veros, boundary_map, label, queue, nerror,iperm, jperm, iofs, nippts, imt, jmt, mnisle, maxipp):
    """
