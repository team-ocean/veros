...
def calc_topo(veros):
    ...
    veros.maskU[:veros.nx+3, :, :] = np.minimum(veros.maskT[:veros.nx+3, :, :], veros.maskT[1:veros.nx+4, :, :])
    if veros.enable_cyclic_x:
    ...
    veros.maskW[:,:,:veros.nz-1] = np.minimum(veros.maskT[:,:,:veros.nz-1], veros.maskT[:,:,1:veros.nz])
...
def calc_initial_conditions(veros):
    ...
    veros.rho[...] = density.get_rho(veros,veros.salt,veros.temp,np.abs(veros.zt)[:,np.newaxis]) * veros.maskT[...,np.newaxis]
    veros.Hd[...] = density.get_dyn_enthalpy(veros,veros.salt,veros.temp,np.abs(veros.zt)[:,np.newaxis]) * veros.maskT[...,np.newaxis]
    veros.int_drhodT[...] = density.get_int_drhodT(veros,veros.salt,veros.temp,np.abs(veros.zt)[:,np.newaxis])
    veros.int_drhodS[...] = density.get_int_drhodS(veros,veros.salt,veros.temp,np.abs(veros.zt)[:,np.newaxis])
...
def ugrid_to_tgrid(veros, a):
    b = np.zeros_like(a)
    b[2:-2,:,:] = (veros.dxu[2:-2, np.newaxis, np.newaxis] * a[2:-2, :, :] + veros.dxu[1:-3, np.newaxis, np.newaxis] * a[1:-3, :, :]) / (2*veros.dxt[2:-2, np.newaxis, np.newaxis])
    return b

@veros_method
def vgrid_to_tgrid(veros, a):
    b = np.zeros_like(a)
    b[:,2:-2,:] = (veros.area_v[:,2:-2,np.newaxis] * a[:,2:-2,:] + veros.area_v[:,1:-3,np.newaxis] * a[:,1:-3,:]) / (2*veros.area_t[:,2:-2,np.newaxis])
    return b
