...
def integrate_eke(veros):
    ...
    if veros.enable_eke_leewave_dissipation:
        ...
        fxa *= 1.5 * fxa / np.sqrt(np.maximum(1e-6, np.abs(veros.coriolis_t[2:-2, 2:-2, np.newaxis]))) - 2
        ...
        """
        a_loc = np.sum(veros.c_Ri_diss[:,:,:-1] * veros.eke[:,:,:-1,veros.tau] * veros.maskW[:,:,:-1] * veros.dzw[:-1], axis=2)
        b_loc = np.sum(veros.eke[:,:,:-1,veros.tau] * veros.maskW[:,:,:-1] * veros.dzw[:-1], axis=2)
        a_loc += veros.c_Ri_diss[:,:,-1] * veros.eke[:,:,-1,veros.tau] * veros.maskW[:,:,-1] * veros.dzw[-1] * 0.5
        ...
        a_loc[2:-2, 2:-2] += np.sum((veros.c_lee[2:-2,2:-2,np.newaxis] * veros.eke[2:-2,2:-2,:,veros.tau] \
                           ...
                           + 2 * veros.eke_r_bot * veros.eke[2:-2,2:-2,:,veros.tau] * math.sqrt(2.0) * veros.sqrteke[2:-2,2:-2,:] \
    ...
    b_tri[:, :, 1:-1] = 1 + (delta[:,:,1:-1] + delta[:,:,:-2]) / veros.dzw[1:-1] + veros.dt_tracer * c_int[2:-2, 2:-2, 1:-1]
    b_tri[:, :, -1] = 1 + delta[:,:,-2] / (0.5 * veros.dzw[-1]) + veros.dt_tracer * c_int[2:-2, 2:-2, -1]
    b_tri_edge = 1 + delta / veros.dzw[np.newaxis, np.newaxis, :] + veros.dt_tracer * c_int[2:-2, 2:-2, :]
    ...
    if veros.enable_eke_leewave_dissipation:
        ...
        """
        a_loc = np.sum((veros.eke_diss_iw[:,:,:-1] + veros.eke_diss_tke[:,:,:-1]) * veros.dzw[np.newaxis, np.newaxis, :-1], axis=2)
        b_loc = np.sum(c_int[:,:,:-1] * veros.eke[:,:,:-1,veros.taup1] * veros.dzw[np.newaxis, np.newaxis, :-1], axis=2)
        ...
        veros.eke_lee_flux[2:-2, 2:-2] = np.where(boundary_mask, np.sum(veros.c_lee[2:-2, 2:-2, np.newaxis] * veros.eke[2:-2, 2:-2, :, veros.taup1] \
                                                        * veros.dzw[np.newaxis, np.newaxis, :] * full_mask, axis=-1)
                                                , veros.eke_lee_flux[2:-2, 2:-2])
        veros.eke_bot_flux[2:-2, 2:-2] = np.where(boundary_mask, np.sum(2 * veros.eke_r_bot * veros.eke[2:-2, 2:-2, :, veros.taup1] \
                                                        * math.sqrt(2.0) * veros.sqrteke[2:-2, 2:-2, :] * full_mask, axis=-1)
                                                , veros.eke_bot_flux[2:-2, 2:-2])
    else:
    ...
    if veros.enable_eke_superbee_advection:
        advection.adv_flux_superbee_wgrid(veros, veros.flux_east, veros.flux_north, veros.flux_top, veros.eke[:,:,:,veros.tau])
    if veros.enable_eke_upwind_advection:
        advection.adv_flux_upwind_wgrid(veros, veros.flux_east, veros.flux_north, veros.flux_top, veros.eke[:,:,:,veros.tau])
    if veros.enable_eke_superbee_advection or veros.enable_eke_upwind_advection:
        ...
        veros.deke[:,:,1:-1,veros.tau] += -(veros.flux_top[:,:,1:-1] - veros.flux_top[:,:,:-2]) / veros.dzw[np.newaxis, np.newaxis, 1:-1]
        veros.deke[:,:,-1,veros.tau] += -(veros.flux_top[:,:,-1] - veros.flux_top[:,:,-2]) / (0.5 * veros.dzw[-1])
        """
