...
@veros_method
def adv_flux_superbee_spectral(veros,is_,ie_,js_,je_,np_,adv_fe,adv_fn,adv_ft,var,uvel,vvel,wvel):
    """
...
@veros_method
def reflect_flux(veros,is_,ie_,js_,je_,np_,adv_fe,adv_fn):
    """
...
def reflect_ini(veros):
    ...
    for k in xrange(2,np-1): # k = 2,np-1
        ...
        if phit[k] >= veros.pi and phit[k] < 2*veros.pi:
            fxa = 2*veros.pi - phit[k]
            if xa < 0.:
                fxa = fxa + 2*pi
            if fxa > 2*pi:
                fxa = fxa - 2*pi
        ...
        if phit[k] >= veros.pi/2 and phit[k] < 3*veros.pi/2.:
            fxa = veros.pi - phit[k]
            if fxa < 0.:
                fxa = fxa + 2*pi
            if fxa > 2*pi:
...
@veros_method
def calc_spectral_topo(veros):
    """
    spectral stuff related to topography
    """
    if veros.enable_idemix_M2 or veros.enable_idemix_niw:
        # wavenumber grid
        dphit = 2.*veros.pi/(np-2)
        dphiu = dphit
        phit[1] = 0.0 - dphit[1]
