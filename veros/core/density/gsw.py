...
"""
==========================================================================
  Potential density & temperature, enthropy, Gibbs free energy and its
...
"""
@veros_method
def get_gsw_teos10_constants(vs, key):
...
@veros_method
def gsw_entropy_part(vs, sa, t, p):
    """
    ...
    x2 = get_gsw_teos10_constants(vs, 'gsw_sfac') * sa
    x = np.sqrt(x2)
    y = t * 0.025
    z = p * 1e-4
...
def gsw_gibbs(vs, nsd, ntd, npd, sa, t, p):
    ...
    gsw_sfac = get_gsw_teos10_constants(vs, 'gsw_sfac')
    x2 = gsw_sfac * sa
    x = np.sqrt(x2)
    y = t * 0.025
    z = p * 1e-4
    ...
    if (nsd==0) and (ntd==0) and (npd==0):
      ...
      if sa > 0.: g08 = g08 + x2*(5812.81456626732 + 851.226734946706*y) * np.log(x)
      ...
      if sa>0: g08 = g08 + 851.226734946706 * x2 * np.log(x)
      ...
      return (g03 + g08) * 2.5e-10
        ...
        if sa>0: g08 = g08 + 1702.453469893412 * np.log(x)
      ...
      raise ValueError("nsd, ntd and npd values do not match (given: {}, {}, {})".\
            format(nsd, ntd, npd))
...
@veros_method
def gsw_pot_rho_t_exact(vs, sa, t, p, p_ref):
    """
    ...
    pt = gsw_pt_from_t(vs, sa, t, p, p_ref)
    return gsw_rho_t_exact(vs, sa, pt, p_ref)

@veros_method
def gsw_pt_from_t(vs, sa, t, p, p_ref):
    """
    ...
    s1 = sa / get_gsw_teos10_constants(vs, 'gsw_ups')
    ...
    dentropy_dt = get_gsw_teos10_constants(vs, 'gsw_cp0') /\
                  ((get_gsw_teos10_constants(vs, 'gsw_t0') + pt) *\
                  (1.0 - 0.05 * (1.0 - sa / get_gsw_teos10_constants(vs, 'gsw_sso'))))

    true_entropy_part = gsw_entropy_part(vs, sa, t, p)

    for _ in range(2):
        pt_old = pt
        dentropy = gsw_entropy_part(vs, sa, pt_old, p_ref) - true_entropy_part
        pt = pt_old - dentropy / dentropy_dt
        ptm = 0.5 * (pt + pt_old)
        dentropy_dt = -gsw_gibbs(vs, 0, 2, 0, sa, ptm, p_ref)
        pt = pt_old - dentropy / dentropy_dt
