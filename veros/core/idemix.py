...
def set_idemix_parameter(state):
    ...
    bN0 = npx.sum(npx.sqrt(npx.maximum(0., vs.Nsqr[:, :, :-1, vs.tau]))
                 * vs.dzw[npx.newaxis, npx.newaxis, :-1] * vs.maskW[:, :, :-1], axis=2) \
        + npx.sqrt(npx.maximum(0., vs.Nsqr[:, :, -1, vs.tau])) \
        * 0.5 * vs.dzw[-1:] * vs.maskW[:, :, -1]
    fxa = npx.sqrt(npx.maximum(0., vs.Nsqr[..., vs.tau])) \
        / (1e-22 + npx.abs(vs.coriolis_t[..., npx.newaxis]))

    cstar = npx.maximum(1e-2, bN0[:, :, npx.newaxis] / (settings.pi * settings.jstar))

    vs.c0 = npx.maximum(0., settings.gamma * cstar * gofx2(fxa, settings.pi) * vs.maskW)
    vs.v0 = npx.maximum(0., settings.gamma * cstar * hofx1(fxa, settings.pi) * vs.maskW)
    vs.alpha_c = npx.maximum(1e-4, settings.mu0 * npx.arccosh(npx.maximum(1., fxa)) * npx.abs(vs.coriolis_t[..., npx.newaxis]) / cstar**2) * vs.maskW
...
def integrate_idemix_kernel(state):
    ...
    if settings.enable_eke and (settings.enable_eke_diss_bottom or settings.enable_eke_diss_surfbot):
        ...
        """
        a_loc = npx.sum(vs.dzw[npx.newaxis, npx.newaxis, :-1] *
        ...
        if settings.enable_eke_diss_bottom:
            forc = update(forc, at[2:-2, 2:-2, :], npx.where(mask, a_loc[2:-2, 2:-2, npx.newaxis] /
                                                  vs.dzw[npx.newaxis, npx.newaxis, :], forc[2:-2, 2:-2, :]))
        else:
            forc = update(forc, at[2:-2, 2:-2, :], npx.where(mask, settings.eke_diss_surfbot_frac * a_loc[2:-2, 2:-2, npx.newaxis]
                                           / vs.dzw[npx.newaxis, npx.newaxis, :], forc[2:-2, 2:-2, :]))
    ...
    """
    maxE_iw = npx.maximum(0., vs.E_iw[:, :, :, vs.tau])
    ...
    delta = update(delta, at[:, :, :-1], settings.dt_tracer * settings.tau_v / vs.dzt[npx.newaxis, npx.newaxis, 1:] * 0.5 \
        * (vs.c0[2:-2, 2:-2, :-1] + vs.c0[2:-2, 2:-2, 1:]))
    delta = update(delta, at[:, :, -1], 0.)
    a_tri = update(a_tri, at[:, :, 1:-1], -delta[:, :, :-2] * vs.c0[2:-2, 2:-2, :-2] \
        / vs.dzw[npx.newaxis, npx.newaxis, 1:-1])
    a_tri = update(a_tri, at[:, :, -1], -delta[:, :, -2] / (0.5 * vs.dzw[-1:]) * vs.c0[2:-2, 2:-2, -2])
    b_tri = update(b_tri, at[:, :, 1:-1], 1 + delta[:, :, 1:-1] * vs.c0[2:-2, 2:-2, 1:-1] / vs.dzw[npx.newaxis, npx.newaxis, 1:-1] \
    ...
    c_tri = update(c_tri, at[:, :, :-1], -delta[:, :, :-1] / \
        vs.dzw[npx.newaxis, npx.newaxis, :-1] * vs.c0[2:-2, 2:-2, 1:])
    d_tri = update(d_tri, at[...], vs.E_iw[2:-2, 2:-2, :, vs.tau] + settings.dt_tracer * forc[2:-2, 2:-2, :])
    d_tri_edge = d_tri + settings.dt_tracer * \
        vs.forc_iw_bottom[2:-2, 2:-2, npx.newaxis] / vs.dzw[npx.newaxis, npx.newaxis, :]
...
def gofx2(x, pi):
    x = npx.maximum(3., x)
    c = 1. - (2. / pi) * npx.arcsin(1. / x)
    return 2. / pi / c * 0.9 * x**(-2. / 3.) * (1 - npx.exp(-x / 4.3))


@veros_kernel
def hofx1(x, pi):
    eps = npx.finfo(x.dtype).eps  # prevent division by zero
    x = npx.maximum(1. + eps, x)
    return (2. / pi) / (1. - (2. / pi) * npx.arcsin(1. / x)) * (x - 1.) / (x + 1.)
