...
def set_idemix_parameter(veros):
    ...
    bN0 = np.sum(np.sqrt(np.maximum(0., veros.Nsqr[:, :, :-1, veros.tau]))
        ...
        + np.sqrt(np.maximum(0., veros.Nsqr[:, :, -1, veros.tau])
                  ) * 0.5 * veros.dzw[-1:] * veros.maskW[:, :, -1]
    fxa = np.sqrt(np.maximum(0., veros.Nsqr[..., veros.tau])) / \
        (1e-22 + np.abs(veros.coriolis_t[..., np.newaxis]))
...
def integrate_idemix(veros):
    ...
    if veros.enable_eke:
        ...
        if veros.enable_store_cabbeling_heat:
            forc[...] = veros.K_diss_gm + veros.K_diss_h - \
                veros.P_diss_skew - veros.P_diss_hmix - veros.P_diss_iso
        else:
    ...
    if veros.enable_eke and (veros.enable_eke_diss_bottom or veros.enable_eke_diss_surfbot):
        ...
        """
        a_loc = np.sum(veros.dzw[np.newaxis, np.newaxis, :-1] *
                       forc[:, :, :-1] * veros.maskW[:, :, :-1], axis=2)
    ...
    delta[:, :, -1] = 0.
    a_tri[:, :, 1:-1] = -delta[:, :, :-2] * veros.c0[2:-2, 2:-2, :-2] \
    ...
    c_tri[:, :, :-1] = -delta[:, :, :-1] / \
        veros.dzw[np.newaxis, np.newaxis, :-1] * veros.c0[2:-2, 2:-2, 1:]
    d_tri[...] = veros.E_iw[2:-2, 2:-2, :, veros.tau] + veros.dt_tracer * forc[2:-2, 2:-2, :]
    d_tri_edge = d_tri + veros.dt_tracer * \
        veros.forc_iw_bottom[2:-2, 2:-2, np.newaxis] / veros.dzw[np.newaxis, np.newaxis, :]
    ...
    if veros.enable_idemix_superbee_advection:
        advection.adv_flux_superbee_wgrid(
            veros, veros.flux_east, veros.flux_north, veros.flux_top, veros.E_iw[:, :, :, veros.tau])
