...
def isoneutral_friction(veros):
    ...
    delta, a_tri, b_tri, c_tri = (np.zeros((veros.nx+1,veros.ny+1,veros.nz)) for _ in range(4))
    delta[:, :, :-1] = veros.dt_mom / veros.dzw[np.newaxis, np.newaxis, :-1] * fxa[:, :, :-1] * veros.maskU[1:-2, 1:-2, 1:] * veros.maskU[1:-2, 1:-2, :-1]
    delta[-1] = 0.
    a_tri[:, :, 1:] = -delta[:, :, :-1] / veros.dzt[np.newaxis, np.newaxis, 1:]
    b_tri_edge = 1 + delta / veros.dzt[np.newaxis, np.newaxis, :]
    b_tri[:, :, 1:-1] = 1 + delta[:, :, 1:-1] / veros.dzt[np.newaxis, np.newaxis, 1:-1] + delta[:, :, :-2] / veros.dzt[np.newaxis, np.newaxis, 1:-1]
    b_tri[:, :, -1] = 1 + delta[:, :, -2] / veros.dzt[-1]
    c_tri[...] = - delta / veros.dzt[np.newaxis, np.newaxis, :]
    sol, water_mask = utilities.solve_implicit(veros, ks, a_tri, b_tri, c_tri, aloc[1:-2, 1:-2, :], b_edge=b_tri_edge)
    veros.u[1:-2, 1:-2, :, veros.taup1] = np.where(water_mask, sol, veros.u[1:-2, 1:-2, :, veros.taup1])
    veros.du_mix[1:-2, 1:-2, :] += (veros.u[1:-2, 1:-2, :, veros.taup1] - aloc[1:-2, 1:-2, :]) / veros.dt_mom * water_mask
    ...
    delta, a_tri, b_tri, c_tri = (np.zeros((veros.nx+1,veros.ny+1,veros.nz)) for _ in range(4))
    delta[:, :, :-1] = veros.dt_mom / veros.dzw[np.newaxis, np.newaxis, :-1] * fxa[:, :, :-1] * veros.maskV[1:-2, 1:-2, 1:] * veros.maskV[1:-2, 1:-2, :-1]
    delta[-1] = 0.
    a_tri[:, :, 1:] = -delta[:, :, :-1] / veros.dzt[np.newaxis, np.newaxis, 1:]
    b_tri_edge = 1 + delta / veros.dzt[np.newaxis, np.newaxis, :]
    b_tri[:, :, 1:-1] = 1 + delta[:, :, 1:-1] / veros.dzt[np.newaxis, np.newaxis, 1:-1] + delta[:, :, :-2] / veros.dzt[np.newaxis, np.newaxis, 1:-1]
    ...
    c_tri[...] = - delta / veros.dzt[np.newaxis, np.newaxis, :]
    sol, water_mask = utilities.solve_implicit(veros, ks, a_tri, b_tri, c_tri, aloc[1:-2, 1:-2, :], b_edge=b_tri_edge)
    veros.v[1:-2, 1:-2, :, veros.taup1] = np.where(water_mask, sol, veros.v[1:-2, 1:-2, :, veros.taup1])
    veros.dv_mix[1:-2, 1:-2, :] += (veros.v[1:-2, 1:-2, :, veros.taup1] - aloc[1:-2, 1:-2, :]) / veros.dt_mom * water_mask
