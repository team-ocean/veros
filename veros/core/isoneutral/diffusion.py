...
def _calc_tracer_fluxes(veros, tr, K_iso, K_skew):
    ...
    veros.flux_east[1:-2, 2:-2, :] = sumz / (4. * veros.dzt[np.newaxis, np.newaxis, :]) + (tr[2:-1, 2:-2, :, veros.tau] - tr[1:-2, 2:-2, :, veros.tau]) / (
        veros.cost[np.newaxis, 2:-2, np.newaxis] * veros.dxu[1:-2, np.newaxis, np.newaxis]) * veros.K_11[1:-2, 2:-2, :]
    ...
    veros.flux_north[2:-2, 1:-2, :] = veros.cosu[np.newaxis, 1:-2, np.newaxis] * (sumz / (4. * veros.dzt[np.newaxis, np.newaxis, :]) + (
        tr[2:-2, 2:-1, :, veros.tau] - tr[2:-2, 1:-2, :, veros.tau]) / veros.dyu[np.newaxis, 1:-2, np.newaxis] * veros.K_22[2:-2, 1:-2, :])
...
def _calc_implicit_part(veros, tr):
    ...
    b_tri[:, :, 1:-1] = 1 + (delta[:, :, 1:-1] + delta[:, :, :-2]) / \
        veros.dzt[np.newaxis, np.newaxis, 1:-1]
    ...
    sol, water_mask = utilities.solve_implicit(
        veros, ks, a_tri, b_tri, c_tri, tr[2:-2, 2:-2, :, veros.taup1], b_edge=b_tri_edge)
...
def isoneutral_diffusion(veros, tr, istemp, iso=True, skew=False):
    ...
    if veros.enable_conserve_energy:
        ...
        if not iso:
            veros.P_diss_skew[2:-2, 2:-2, :-1] += - veros.grav / veros.rho_0 * \
                fxa * veros.flux_top[2:-2, 2:-2, :-1] * veros.maskW[2:-2, 2:-2, :-1]
        else:
