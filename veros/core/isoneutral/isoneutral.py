...
def isoneutral_diffusion_pre(veros):
    ...
    """
    drdTS[:,:,:,0] = density.get_drhodT(veros,veros.salt[:,:,:,veros.tau],veros.temp[:,:,:,veros.tau],np.abs(veros.zt)) * veros.maskT
    drdTS[:,:,:,1] = density.get_drhodS(veros,veros.salt[:,:,:,veros.tau],veros.temp[:,:,:,veros.tau],np.abs(veros.zt)) * veros.maskT

    """
    gradients at top face of T cells
    """
    ddzt[:,:,:-1,0] = veros.maskW[:,:,:-1] * (veros.temp[:,:,1:,veros.tau] - veros.temp[:,:,:-1,veros.tau]) / veros.dzw[np.newaxis,np.newaxis,:-1]
    ddzt[:,:,:-1,1] = veros.maskW[:,:,:-1] * (veros.salt[:,:,1:,veros.tau] - veros.salt[:,:,:-1,veros.tau]) / veros.dzw[np.newaxis,np.newaxis,:-1]
    ddzt[...,-1,:] = 0.

    """
    gradients at eastern face of T cells
    """
    ddxt[:-1,:,:,0] = veros.maskU[:-1,:,:] * (veros.temp[1:,:,:,veros.tau] - veros.temp[:-1,:,:,veros.tau]) / (veros.dxu[:-1,np.newaxis,np.newaxis] * veros.cost[np.newaxis,:,np.newaxis])
    ddxt[:-1,:,:,1] = veros.maskU[:-1,:,:] * (veros.salt[1:,:,:,veros.tau] - veros.salt[:-1,:,:,veros.tau]) / (veros.dxu[:-1,np.newaxis,np.newaxis] * veros.cost[np.newaxis,:,np.newaxis])

    """
    gradients at northern face of T cells
    """
    ddyt[:,:-1,:,0] = veros.maskV[:,:-1,:] * (veros.temp[:,1:,:,veros.tau] - veros.temp[:,:-1,:,veros.tau]) / veros.dyu[np.newaxis,:-1,np.newaxis]
    ddyt[:,:-1,:,1] = veros.maskV[:,:-1,:] * (veros.salt[:,1:,:,veros.tau] - veros.salt[:,:-1,:,veros.tau]) / veros.dyu[np.newaxis,:-1,np.newaxis]

    """
    Compute Ai_ez and K11 on center of east face of T cell.
    """
    diffloc = np.zeros((veros.nx+4, veros.ny+4, veros.nz))
    diffloc[1:-2,2:-2,1:] = 0.25 * (veros.K_iso[1:-2,2:-2,1:] + veros.K_iso[1:-2,2:-2,:-1] + veros.K_iso[2:-1,2:-2,1:] + veros.K_iso[2:-1,2:-2,:-1])
    ...
    for kr in xrange(2):
        ...
        for ip in xrange(2):
            ...
            sumz[:,:,ki:] += veros.dzw[np.newaxis,np.newaxis,:-1+kr or None] * veros.maskU[1:-2,2:-2,ki:] * np.maximum(veros.K_iso_steep, diffloc[1:-2,2:-2,ki:]*taper)
    ...
    """
    diffloc = np.zeros((veros.nx+4, veros.ny+4, veros.nz))
    diffloc[2:-2, 1:-2, 1:] = 0.25 * (veros.K_iso[2:-2,1:-2,1:] + veros.K_iso[2:-2,1:-2,:-1] + veros.K_iso[2:-2,2:-1,1:] + veros.K_iso[2:-2,2:-1,:-1])
    ...
    for kr in xrange(2):
        ...
        for jp in xrange(2):
            ...
            sumz[:,:,ki:] += veros.dzw[np.newaxis,np.newaxis,:-1+kr or None] * veros.maskV[2:-2,1:-2,ki:] * np.maximum(veros.K_iso_steep, diffloc[2:-2,1:-2,ki:]*taper)
    ...
    for ip in xrange(2):
        for kr in xrange(2):
            ...
            sumx += veros.dxu[1+ip:-3+ip,np.newaxis,np.newaxis] * veros.K_iso[2:-2,2:-2,:-1] * taper * sxb**2 * veros.maskW[2:-2,2:-2,:-1]
    ...
    for jp in xrange(2): # jp=0,1
        ...
        for kr in xrange(2): # kr=0,1
            ...
            sumy += facty[np.newaxis,:,np.newaxis] * veros.K_iso[2:-2,2:-2,:-1] * taper * syb**2 * veros.maskW[2:-2,2:-2,:-1]
            veros.Ai_by[2:-2,2:-2,:-1,jp,kr] = taper * syb * veros.maskW[2:-2,2:-2,:-1]

    veros.K_33[2:-2,2:-2,:-1] = sumx / (4*veros.dxt[2:-2,np.newaxis,np.newaxis]) + sumy / (4*veros.dyt[np.newaxis,2:-2,np.newaxis] * veros.cost[np.newaxis,2:-2,np.newaxis])
...
def check_isoneutral_slope_crit(veros):
    ...
    if veros.enable_neutral_diffusion:
        ...
        delta1a = np.min(veros.dxt[2:-2, np.newaxis, np.newaxis] * np.abs(veros.cost[np.newaxis, 2:-2, np.newaxis]) * veros.dzt[np.newaxis, np.newaxis, :] * ft1)
        delta1b = np.min(veros.dyt[np.newaxis, 2:-2, np.newaxis] * veros.dzt[np.newaxis, np.newaxis, :] * ft1)
        delta_iso1 = np.minimum(veros.dzt[0] * ft1 * veros.dxt[-1] * np.abs(veros.cost[-1]), np.minimum(delta1a, delta1b))
