...
def thermodynamics(veros):
    ...
    if veros.enable_conserve_energy:
        ...
        if veros.enable_superbee_advection:
            advection.adv_flux_superbee(veros,veros.flux_east,veros.flux_north,veros.flux_top,veros.Hd[:,:,:,veros.tau])
        else:
            advection.adv_flux_2nd(veros,veros.flux_east,veros.flux_north,veros.flux_top,veros.Hd[:,:,:,veros.tau])
        ...
        veros.dHd[:,:,0,veros.tau] += -veros.maskT[:,:,0] * veros.flux_top[:,:,0] / veros.dzt[0]
        veros.dHd[:,:,1:,veros.tau] += -veros.maskT[:,:,1:] * (veros.flux_top[:,:,1:] - veros.flux_top[:,:,:-1]) / veros.dzt[np.newaxis, np.newaxis, 1:]
    ...
    veros.temp[:,:,:,veros.taup1] = veros.temp[:,:,:,veros.tau] + veros.dt_tracer * \
                    ((1.5+veros.AB_eps)*veros.dtemp[:,:,:,veros.tau] - (0.5+veros.AB_eps)*veros.dtemp[:,:,:,veros.taum1]) * veros.maskT
    veros.salt[:,:,:,veros.taup1] = veros.salt[:,:,:,veros.tau] + veros.dt_tracer * \
                    ((1.5+veros.AB_eps)*veros.dsalt[:,:,:,veros.tau] - (0.5+veros.AB_eps)*veros.dsalt[:,:,:,veros.taum1]) * veros.maskT
    ...
    with veros.timers["vmix"]:
        ...
        delta[:, :, :-1] = veros.dt_tracer / veros.dzw[np.newaxis, np.newaxis, :-1] * veros.kappaH[2:-2, 2:-2, :-1]
        delta[:, :, -1] = 0.
        a_tri[:, :, 1:] = -delta[:,:,:-1] / veros.dzt[np.newaxis, np.newaxis, 1:]
        b_tri[:, :, 1:] = 1 + (delta[:, :, 1:] + delta[:, :, :-1]) / veros.dzt[np.newaxis, np.newaxis, 1:]
        ...
        sol, mask = utilities.solve_implicit(veros, ks, a_tri, b_tri, c_tri, d_tri, b_edge=b_tri_edge)
        veros.salt[2:-2, 2:-2, :, veros.taup1] = np.where(mask, sol, veros.salt[2:-2, 2:-2, :, veros.taup1])

        veros.dtemp_vmix[...] = (veros.temp[:,:,:,veros.taup1] - veros.dtemp_vmix) / veros.dt_tracer
        veros.dsalt_vmix[...] = (veros.salt[:,:,:,veros.taup1] - veros.dsalt_vmix) / veros.dt_tracer
    ...
    veros.forc_rho_surface[...] = (
                                density.get_drhodT(veros,veros.salt[:,:,-1,veros.taup1],veros.temp[:,:,-1,veros.taup1],np.abs(veros.zt[-1])) * veros.forc_temp_surface \
                              + density.get_drhodS(veros,veros.salt[:,:,-1,veros.taup1],veros.temp[:,:,-1,veros.taup1],np.abs(veros.zt[-1])) * veros.forc_salt_surface \
                            ) * veros.maskT[:,:,-1]
    ...
    with veros.timers["vmix"]:
        ...
        if veros.enable_conserve_energy:
            ...
            """
            fxa = (-veros.int_drhodT[2:-2, 2:-2, 1:, veros.taup1] + veros.int_drhodT[2:-2, 2:-2, :-1,veros.taup1]) / veros.dzw[np.newaxis, np.newaxis, :-1]
            veros.P_diss_v[2:-2, 2:-2, :-1] += -veros.grav / veros.rho_0 * fxa * veros.kappaH[2:-2, 2:-2, :-1] \
                                              * (veros.temp[2:-2, 2:-2, 1:, veros.taup1] - veros.temp[2:-2, 2:-2, :-1,veros.taup1]) \
                                              / veros.dzw[np.newaxis, np.newaxis, :-1] * veros.maskW[2:-2, 2:-2, :-1]
            fxa = (-veros.int_drhodS[2:-2, 2:-2, 1:, veros.taup1] + veros.int_drhodS[2:-2, 2:-2, :-1,veros.taup1]) / veros.dzw[np.newaxis, np.newaxis, :-1]
            veros.P_diss_v[2:-2, 2:-2, :-1] += -veros.grav / veros.rho_0 * fxa * veros.kappaH[2:-2, 2:-2, :-1] \
            ...
            veros.P_diss_v[2:-2, 2:-2, -1] += - veros.grav / veros.rho_0 * fxa * veros.forc_temp_surface[2:-2 ,2:-2] * veros.maskW[2:-2, 2:-2, -1]
            fxa = 2 * veros.int_drhodS[2:-2, 2:-2, -1, veros.taup1] / veros.dzw[-1]
            veros.P_diss_v[2:-2, 2:-2, -1] += - veros.grav / veros.rho_0 * fxa * veros.forc_salt_surface[2:-2 ,2:-2] * veros.maskW[2:-2, 2:-2, -1]
        ...
        if veros.enable_conserve_energy:
            ...
            veros.P_diss_v[:,:,-1] = -veros.forc_rho_surface * veros.maskT[:,:,-1] * veros.grav / veros.rho_0
...
def advect_tracer(veros, tr, dtr):
    ...
    dtr[:, :, 1:] += -veros.maskT[:, :, 1:] * (veros.flux_top[:, :, 1:] - veros.flux_top[:, :, :-1]) / veros.dzt[1:]
...
def calc_eq_of_state(veros, n):
    ...
    """
    fxa = -veros.grav / veros.rho_0 / veros.dzw[np.newaxis, np.newaxis, :-1] * veros.maskW[:, :, :-1]
    veros.Nsqr[:, :, :-1, n] = fxa * (density.get_rho(veros, veros.salt[:,:,1:,n], veros.temp[:,:,1:,n], np.abs(veros.zt[:-1])) - veros.rho[:,:,:-1,n])
