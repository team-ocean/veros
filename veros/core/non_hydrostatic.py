...
def solve_non_hydrostatic(veros):
    ...
    solve for non-hydrostatic pressure
    """
    veros.p_non_hydro[:, :, :, veros.taup1] = 2 * veros.p_non_hydro[:, :, :,
                                                                    veros.tau] - veros.p_non_hydro[:, :, :, veros.taum1]  # first guess
    ...
    for k in xrange(veros.nz):  # k = 1,nz
        for j in xrange(veros.js_pe, veros.je_pe):  # j = js_pe,je_pe
            for i in xrange(veros.is_pe, veros.ie_pe):  # i = is_pe,ie_pe
                ...
                cf[i, j, k, 1, 1, 1] += -mp / veros.dyu[j] / \
                    veros.dyt[j] * veros.cosu[j] / veros.cost[j]
                cf[i, j, k, 1, 2, 1] += mp / veros.dyu[j] / \
                    veros.dyt[j] * veros.cosu[j] / veros.cost[j]
                cf[i, j, k, 1, 1, 1] += -mm / veros.dyu[j - 1] / \
                    veros.dyt[j] * veros.cosu[j - 1] / veros.cost[j]
                cf[i, j, k, 1, 0, 1] += mm / veros.dyu[j - 1] / \
                    veros.dyt[j] * veros.cosu[j - 1] / veros.cost[j]
