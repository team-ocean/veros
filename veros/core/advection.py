...
def _calc_cr(rjp, rj, rjm, vel):
    ...
    return npx.where(vel > 0., rjm, rjp) / npx.where(npx.abs(rj) < eps, eps, rj)
...
def _adv_superbee(state, vel, var, mask, dx, axis):
    ...
    return vel[s] * (var[sp1] + var[s]) * 0.5 - npx.abs(vel[s]) * ((1. - cr) + uCFL * cr) * rj * 0.5
...
def adv_flux_2nd(state, var):
    ...
    adv_fn = update(adv_fn, at[2:-2, 1:-2, :], vs.cosu[npx.newaxis, 1:-2, npx.newaxis] * 0.5 * (var[2:-2, 1:-2, :] + var[2:-2, 2:-1, :]) \
...
def calculate_velocity_on_wgrid_kernel(state):
    ...
    vs.u_wgrid = update(vs.u_wgrid, at[:, :, :-1], vs.u[:, :, 1:, vs.tau] * vs.maskU[:, :, 1:] * 0.5 \
        * vs.dzt[npx.newaxis, npx.newaxis, 1:] / vs.dzw[npx.newaxis, npx.newaxis, :-1] \
    ...
    vs.u_wgrid = update_add(vs.u_wgrid, at[:-1, :, 1:], (vs.u_wgrid[:-1, :, :-1] * vs.dzw[npx.newaxis, npx.newaxis, :-1]
                            / vs.dzw[npx.newaxis, npx.newaxis, 1:]) * (1. - mask))
    vs.u_wgrid = update_multiply(vs.u_wgrid, at[:-1, :, :-1], mask)
    mask = vs.maskW[:, :-1, :-1] * vs.maskW[:, 1:, :-1]
    vs.v_wgrid = update_add(vs.v_wgrid, at[:, :-1, 1:], (vs.v_wgrid[:, :-1, :-1] * vs.dzw[npx.newaxis, npx.newaxis, :-1]
                            / vs.dzw[npx.newaxis, npx.newaxis, 1:]) * (1. - mask))
    ...
    # TODO: replace cumsum
    vs.w_wgrid = update(vs.w_wgrid, at[1:, 1:, :], npx.cumsum(-vs.dzw[npx.newaxis, npx.newaxis, :] *
                                   ((vs.u_wgrid[1:, 1:, :] - vs.u_wgrid[:-1, 1:, :]) / (vs.cost[npx.newaxis, 1:, npx.newaxis] * vs.dxt[1:, npx.newaxis, npx.newaxis])
                                    + (vs.cosu[npx.newaxis, 1:, npx.newaxis] * vs.v_wgrid[1:, 1:, :] -
                                       vs.cosu[npx.newaxis, :-1, npx.newaxis] * vs.v_wgrid[1:, :-1, :])
                                    / (vs.cost[npx.newaxis, 1:, npx.newaxis] * vs.dyt[npx.newaxis, 1:, npx.newaxis])), axis=2))
...
def adv_flux_upwind_wgrid(state, var):
    ...
    adv_fn = update(adv_fn, at[2:-2, 1:-2, :], vs.cosu[npx.newaxis, 1:-2, npx.newaxis] * vs.v_wgrid[2:-2, 1:-2, :] * \
