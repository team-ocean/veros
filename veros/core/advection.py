...
def adv_flux_2nd(veros,adv_fe,adv_fn,adv_ft,var):
    ...
    """
    adv_fe[1:-2, 2:-2, :] = 0.5*(var[1:-2, 2:-2, :] + var[2:-1, 2:-2, :]) * veros.u[1:-2, 2:-2, :, veros.tau] * veros.maskU[1:-2, 2:-2, :]
    ...
    adv_ft[2:-2, 2:-2, :-1] = 0.5 * (var[2:-2, 2:-2, :-1] + var[2:-2, 2:-2, 1:]) * veros.w[2:-2, 2:-2, :-1, veros.tau] * veros.maskW[2:-2, 2:-2, :-1]
    adv_ft[:,:,-1] = 0.
...
def adv_flux_superbee(veros,adv_fe,adv_fn,adv_ft,var):
    ...
    the slope ratio.
    """
    adv_fe[1:-2, 2:-2, :] = _adv_superbee(veros, veros.u[..., veros.tau], var, veros.maskU, veros.dxt, 0)
    adv_fn[2:-2, 1:-2, :] = _adv_superbee(veros, veros.v[..., veros.tau], var, veros.maskV, veros.dyt, 1)
    adv_ft[2:-2, 2:-2, :-1] = _adv_superbee(veros, veros.w[..., veros.tau], var, veros.maskW, veros.dzt, 2)
    adv_ft[..., -1] = 0.
...
def calculate_velocity_on_wgrid(veros):
    ...
    veros.u_wgrid[:,:,:-1] = veros.u[:,:,1:,veros.tau] * veros.maskU[:,:,1:] * 0.5 * veros.dzt[np.newaxis,np.newaxis,1:] / veros.dzw[np.newaxis,np.newaxis,:-1] \
                          + veros.u[:,:,:-1,veros.tau] * veros.maskU[:,:,:-1] * 0.5 * veros.dzt[np.newaxis,np.newaxis,:-1] / veros.dzw[np.newaxis,np.newaxis,:-1]
    veros.v_wgrid[:,:,:-1] = veros.v[:,:,1:,veros.tau] * veros.maskV[:,:,1:] * 0.5 * veros.dzt[np.newaxis,np.newaxis,1:] / veros.dzw[np.newaxis,np.newaxis,:-1] \
                          + veros.v[:,:,:-1,veros.tau] * veros.maskV[:,:,:-1] * 0.5 * veros.dzt[np.newaxis,np.newaxis,:-1] / veros.dzw[np.newaxis,np.newaxis,:-1]
    veros.u_wgrid[:,:,-1] = veros.u[:,:,-1,veros.tau] * veros.maskU[:,:,-1] * 0.5 * veros.dzt[-1:] / veros.dzw[-1:]
    veros.v_wgrid[:,:,-1] = veros.v[:,:,-1,veros.tau] * veros.maskV[:,:,-1] * 0.5 * veros.dzt[-1:] / veros.dzw[-1:]

    # redirect velocity at bottom and at topography
    veros.u_wgrid[:,:,0] = veros.u_wgrid[:,:,0] + veros.u[:,:,0,veros.tau] * veros.maskU[:,:,0] * 0.5 * veros.dzt[0:1] / veros.dzw[0:1]
    veros.v_wgrid[:,:,0] = veros.v_wgrid[:,:,0] + veros.v[:,:,0,veros.tau] * veros.maskV[:,:,0] * 0.5 * veros.dzt[0:1] / veros.dzw[0:1]
    mask = veros.maskW[:-1, :, :-1] * veros.maskW[1:, :, :-1]
    veros.u_wgrid[:-1, :, 1:] += (veros.u_wgrid[:-1, :, :-1] * veros.dzw[np.newaxis, np.newaxis, :-1] / veros.dzw[np.newaxis, np.newaxis, 1:]) * (1.-mask)
    veros.u_wgrid[:-1, :, :-1] *= mask
    mask = veros.maskW[:, :-1, :-1] * veros.maskW[:, 1:, :-1]
    veros.v_wgrid[:, :-1, 1:] += (veros.v_wgrid[:, :-1, :-1] * veros.dzw[np.newaxis, np.newaxis, :-1] / veros.dzw[np.newaxis, np.newaxis, 1:]) * (1.-mask)
    ...
    veros.w_wgrid[1:, 1:, :] = np.cumsum(-veros.dzw[np.newaxis, np.newaxis, :] * \
                              ((veros.u_wgrid[1:, 1:, :] - veros.u_wgrid[:-1, 1:, :]) / (veros.cost[np.newaxis, 1:, np.newaxis] * veros.dxt[1:, np.newaxis, np.newaxis]) \
                               + (veros.cosu[np.newaxis, 1:, np.newaxis] * veros.v_wgrid[1:, 1:, :] - veros.cosu[np.newaxis, :-1, np.newaxis] * veros.v_wgrid[1:, :-1, :]) \
