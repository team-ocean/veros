...
def dissipation_on_wgrid(veros, p_arr, int_drhodX=None, aloc=None, ks=None):
    if aloc is None:
        ...
        aloc[1:-1, 1:-1, :] = 0.5 * veros.grav / veros.rho_0 \
            * ((int_drhodX[2:, 1:-1, :] - int_drhodX[1:-1, 1:-1, :]) * veros.flux_east[1:-1, 1:-1, :]
    ...
    edge_mask = land_mask[:, :, np.newaxis] & (
        np.arange(veros.nz - 1)[np.newaxis, np.newaxis, :] == ks[:, :, np.newaxis])
    ...
    p_arr[:, :, :-1] += (0.5 * (aloc[:, :, :-1] + aloc[:, :, 1:])
                         + 0.5 * (aloc[:, :, :-1] * dzw_pad[np.newaxis, np.newaxis, :-3]
                                  / veros.dzw[np.newaxis, np.newaxis, :-1])) * edge_mask
...
def tempsalt_biharmonic(veros):
    ...
    veros.flux_east[:-1, :, :] = fxa * (del2[1:, :, :] - del2[:-1, :, :]) \
        / (veros.cost[np.newaxis, :, np.newaxis] * veros.dxu[:-1, np.newaxis, np.newaxis]) \
        * veros.maskU[:-1, :, :]
    veros.flux_north[:, :-1, :] = fxa * (del2[:, 1:, :] - del2[:, :-1, :]) \
        / veros.dyu[np.newaxis, :-1, np.newaxis] * veros.maskV[:, :-1, :] \
        * veros.cosu[np.newaxis, :-1, np.newaxis]
    veros.flux_east[-1, :, :] = 0.
    veros.flux_north[:, -1, :] = 0.
...
def tempsalt_diffusion(veros):
    ...
    if veros.enable_hor_friction_cos_scaling:
        ...
        veros.flux_north[...] *= veros.cosu[np.newaxis,
                                            :, np.newaxis] ** veros.hor_friction_cosPower
...
def tempsalt_sources(veros):
    ...
    if veros.enable_conserve_energy:
        aloc = -veros.grav / veros.rho_0 * veros.maskT * \
            (veros.int_drhodT[..., veros.tau] * veros.temp_source +
             veros.int_drhodS[..., veros.tau] * veros.salt_source)
