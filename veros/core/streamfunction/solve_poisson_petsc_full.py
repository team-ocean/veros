...
def initialize_solver(vs):
    ...
    matrix, boundary_fac = _assemble_poisson_matrix(vs, da)

    ksp = PETSc.KSP()
    ksp.create(rst.mpi_comm)
    ksp.setType('gmres')
    ksp.getPC().setType('jacobi')
    ksp.setOperators(matrix)
    ksp.setTolerances(rtol=0, atol=vs.congr_epsilon, max_it=vs.congr_max_iterations)

    rhs_petsc = da.createGlobalVec()
    sol_petsc = da.createGlobalVec()

    def petsc_solver(rhs, x0):
        # add dirichlet BC to rhs
        if not vs.enable_cyclic_x:
            if rst.proc_idx[0] == rs.num_proc[0] - 1:
                rhs[-2, 2:-2] -= rhs[-1, 2:-2] * boundary_fac["east"]

            if rst.proc_idx[0] == 0:
                rhs[2, 2:-2] -= rhs[1, 2:-2] * boundary_fac["west"]

        if rst.proc_idx[1] == rs.num_proc[1] - 1:
            rhs[2:-2, -2] -= rhs[2:-2, -1] * boundary_fac["north"]

        if rst.proc_idx[1] == 0:
            rhs[2:-2, 2] -= rhs[2:-2, 1] * boundary_fac["south"]

        if rs.backend == "bohrium":
            da.getVecArray(rhs_petsc)[...] = np.interop_numpy.get_array(rhs[2:-2, 2:-2].copy())
            da.getVecArray(sol_petsc)[...] = np.interop_numpy.get_array(x0[2:-2, 2:-2].copy())
        else:
            da.getVecArray(rhs_petsc)[...] = rhs[2:-2, 2:-2]
            da.getVecArray(sol_petsc)[...] = x0[2:-2, 2:-2]
        ...
        return np.array(da.getVecArray(sol_petsc)[...])

    vs.petsc_solver = petsc_solver
...
def solve(vs, rhs, sol, boundary_val=None):
    ...
    sol[...] = boundary_val
    sol[2:-2, 2:-2] = vs.petsc_solver(rhs, sol)
...
@veros_method
def _assemble_poisson_matrix(vs, da):
    """
