...
def prepare_forcing(state):
    ...
    vs.p_hydro = update(vs.p_hydro, at[:, :, -2::-1], vs.maskT[:, :, -2::-1] * \
        (vs.p_hydro[:, :, -1, npx.newaxis] + npx.cumsum(ploc[:, :, -2::-1], axis=2)))
    ...
    uloc = npx.sum((vs.du[:, :, :, vs.tau] + vs.du_mix)
                 * vs.maskU * vs.dzt, axis=(2,)) * vs.hur
    vloc = npx.sum((vs.dv[:, :, :, vs.tau] + vs.dv_mix)
    ...
    forc = update(forc, at[2:-2, 2:-2], (vloc[3:-1, 2:-2] - vloc[2:-2, 2:-2]) \
        / (vs.cosu[2:-2] * vs.dxu[2:-2, npx.newaxis]) \
...
def barotropic_velocity_update(state, uloc, vloc):
    ...
    if state.dimensions["isle"] > 1:
        ...
        line_forc = update(line_forc, at[1:], line_integrals.line_integrals(
            state,
            uloc=uloc[..., npx.newaxis],
            vloc=vloc[..., npx.newaxis], kind='same')[1:])
        ...
        uloc = update(uloc, at[1:, 1:], -1 * vs.maskU[1:, 1:, -1] \
            * (vs.dpsi[1:, 1:, vs.taup1] - vs.dpsi[1:, :-1, vs.taup1]) \
            / vs.dyt[npx.newaxis, 1:] * vs.hur[1:, 1:])
        vloc = update(vloc, at[1:, 1:], vs.maskV[1:, 1:, -1] \
            * (vs.dpsi[1:, 1:, vs.taup1] - vs.dpsi[:-1, 1:, vs.taup1]) \
            / (vs.cosu[npx.newaxis, 1:] * vs.dxt[1:, npx.newaxis]) * vs.hvr[1:, 1:])
    ...
    vs.psi = update_add(vs.psi, at[:, :, vs.taup1], settings.dt_mom * npx.sum(((1.5 + settings.AB_eps) * vs.dpsin[1:, vs.tau]
    ...
    vs.u = update_add(vs.u, at[:, :, :, vs.taup1], -uloc[:, :, npx.newaxis] * \
        vs.maskU * vs.hur[:, :, npx.newaxis])
    vs.v = update_add(vs.v, at[:, :, :, vs.taup1], -vloc[:, :, npx.newaxis] * \
        vs.maskV * vs.hvr[:, :, npx.newaxis])
