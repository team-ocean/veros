...
!=======================================================================
! global 4x4 deg model with 45 levels
!======================================================================= 
...
subroutine set_parameter
 ...
 implicit none
  ...
  nz   = 45
  dt_mom    = 3600.0
  !dt_tracer = 86400./6.0  ! with idemix2 else 86400
  dt_tracer = 86400
  ...
  enable_diag_ts_monitor = .true.; ts_monint = 365*86400./48.
  enable_diag_snapshots  = .true.; snapint  =  365*86400./48.0
  !enable_diag_snapshots  = .true.; snapint  =  86400!.*3
  !enable_diag_energy     = .true.; energint = 86400. ;energfreq = dt_tracer
  enable_diag_energy     = .true.; energint = 365*86400./48.0; energfreq = energint
  !enable_diag_overturning= .true.; overint  =  365*86400./24.0; overfreq = dt_tracer*10
  !enable_diag_averages   = .true.; aveint  = 365*86400.*10; avefreq = dt_tracer*10
...
subroutine set_initial_conditions
 ...
 iret=nf_open('forcing_4deg.cdf',NF_NOWRITE,ncid)
 ...
 if (enable_eke .and. enable_eke_leewave_dissipation ) then
  ...
  iret = nf_open('hrms_4deg.nc',NF_nowrite,ncid)
  iret = nf_inq_varid(ncid,'HRMS',id)
  iret = nf_get_vara_double(ncid,id ,(/is_pe,js_pe/),(/ie_pe-is_pe+1,je_pe-js_pe+1/),eke_topo_hrms(is_pe:ie_pe,js_pe:je_pe))
  iret = nf_inq_varid(ncid,'LAM',id)
  iret = nf_get_vara_double(ncid,id ,(/is_pe,js_pe/),(/ie_pe-is_pe+1,je_pe-js_pe+1/),eke_topo_lam(is_pe:ie_pe,js_pe:je_pe))
  call ncclos (ncid, iret)

   call border_exchg_xy(is_pe-onx,ie_pe+onx,js_pe-onx,je_pe+onx,eke_topo_hrms) 
   call setcyclic_xy   (is_pe-onx,ie_pe+onx,js_pe-onx,je_pe+onx,eke_topo_hrms)
   call border_exchg_xy(is_pe-onx,ie_pe+onx,js_pe-onx,je_pe+onx,eke_topo_lam) 
   call setcyclic_xy   (is_pe-onx,ie_pe+onx,js_pe-onx,je_pe+onx,eke_topo_lam)
 ...
 if (enable_idemix ) then
  ...
  if (enable_idemix_niw) omega_niw = max(1d-8, abs( 1.05 * coriolis_t ) )
...
subroutine set_topography
 ...
 iret=nf_open('topo_4deg.cdf',NF_NOWRITE,ncid)
 if (iret /=0 ) then; print*,nf_strerror(iret) ; stop; endif
