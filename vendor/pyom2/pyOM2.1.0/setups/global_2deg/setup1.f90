...
!=======================================================================
! global 2x2 deg model with 15 levels
!======================================================================= 
...
subroutine set_initial_conditions
 ...
 iret=nf_open('ecmwf_2deg_monthly.cdf',NF_NOWRITE,ncid)
 if (iret /=0 ) then; print*,nf_strerror(iret) ; stop; endif

 iret=nf_inq_varid(ncid,'H3',id)
 if (iret /=0 ) then; print*,nf_strerror(iret) ; stop; endif
 iret= nf_get_vara_double(ncid,id,(/is_pe,js_pe,1/), (/ie_pe-is_pe+1,je_pe-js_pe+1,12/),qnet(is_pe:ie_pe,js_pe:je_pe,:))
 if (iret /=0 ) then; print*,nf_strerror(iret) ; stop; endif

 iret=nf_inq_varid(ncid,'Q3',id)
 if (iret /=0 ) then; print*,nf_strerror(iret) ; stop; endif
 iret= nf_get_vara_double(ncid,id,(/is_pe,js_pe,1/), (/ie_pe-is_pe+1,je_pe-js_pe+1,12/),qnec(is_pe:ie_pe,js_pe:je_pe,:))
 if (iret /=0 ) then; print*,nf_strerror(iret) ; stop; endif
 iret = nf_close (ncid)
 do n=1,12
 ...
 open(10,file='tren_taux.bin',access='direct',recl=4*nx*ny,status='old')
 open(20,file='tren_tauy.bin',access='direct',recl=4*nx*ny,status='old')
 do n=1,12
  read(10,rec=n) dat4
 ...
 open(10,file='lev_monthly_temp.bin',access='direct',recl=4*nx*ny,status='old')
 open(20,file='lev_monthly_salt.bin',access='direct',recl=4*nx*ny,status='old')
 do n=1,12
  read(10,rec=n) dat4
...
subroutine set_topography
 ...
 kbot=0
 open(10,file='bathy.bin',access='direct',recl=4*nx*ny,status='old')
 read(10,rec=1) bathy
 close(10)
